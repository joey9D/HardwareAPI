...
static uart_stm32c0 *obj_list[uarts];
...
uart_stm32c0::uart_stm32c0(uint8_t uart, uint32_t baudrate, enum stopbits stopbits, enum parity parity,
    dma_stm32c0 &dma_tx, dma_stm32c0 &dma_rx, gpio_stm32c0 &gpio_tx, gpio_stm32c0 &gpio_rx):
    baud(baudrate),
    stopbits(stopbits),
    parity(parity),
    tx_dma(dma_tx),
    ...
    rx_irq_res(res::ok)
{
    ...
    assert(baud > 0);
    assert(tx_dma.direction() == dma_stm32c0::direction::memory_to_periph);
    assert(tx_dma.increment_size() == 8);
    assert(rx_dma.direction() == dma_stm32c0::direction::periph_to_memory);
    ...
    *rcc_rst_reg[this->uart] &= ~rcc_rst[this->uart];
    ...
    tx_dma.set_callback([this](dma_stm32c0::event event) { on_dma_tx(event); });
    rx_dma.set_callback([this](dma_stm32c0::event event) { on_dma_rx(event); });
...
void uart_stm32c0::baudrate(uint32_t baudrate)
{
    assert(baudrate > 0);
...
uart::res uart_stm32c0::write(const void *buff, uint16_t size)
{
    assert(buff);
    assert(size > 0);
...
uart::res uart_stm32c0::read(void *buff, uint16_t *size, std::chrono::milliseconds timeout)
{
    assert(buff);
    assert(size);
    assert(*size > 0);
...
uart::res uart_stm32c0::write_read(const void *write_buff, uint16_t write_size,
...
void uart_stm32c0::gpio_af_init(gpio_stm32c0 &gpio)
{
...
void uart_stm32c0::on_dma_tx(dma_stm32c0::event event)
{
...
void uart_stm32c0::on_dma_rx(dma_stm32c0::event event)
{
    if(event == dma_stm32c0::event::half_complete)
    {
        return;
...
extern "C" void uart_irq_hndlr(uart_stm32c0 *obj)
{
