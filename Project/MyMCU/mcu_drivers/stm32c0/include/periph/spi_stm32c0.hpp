...
public:
     ...
     */
    spi_stm32c0(uint8_t spi, uint32_t baudrate, enum cpol cpol, enum cpha cpha,
    ...
    spi_stm32c0(const spi_stm32c0&) = delete;
    spi_stm32c0& operator=(const spi_stm32c0&) = delete;

    // Delete move constructor and move assignment operator
    spi_stm32c0(spi_stm32c0&&) = delete;
    spi_stm32c0& operator=(spi_stm32c0&&) = delete;
...
private:
    ...
    TaskHandle_t task;
    enum res irq_res;
    gpio_stm32c0 &mosi, &miso, &clk;
    gpio *cs;
    dma_stm32c0 &write_dma;
    const void *write_buff;
    dma_stm32c0 &read_dma;
    void *read_buff;

    void gpio_af_init(gpio_stm32c0 &gpio);
    static void remap_dma(uint8_t uart, dma_stm32c0 &dma);
    static uint8_t calc_presc(uint8_t spi, uint32_t baud);
    void on_dma_write(dma_stm32c0::event event);
    void on_dma_read(dma_stm32c0::event event);
    friend void ::spi_irq_hndlr(spi_stm32c0 *obj);
};
} // namespace periph
