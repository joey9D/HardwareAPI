...
extern "C" {
    ...
    int Hardware_GPIO_Read(HardwareInstance instance, const PinInfo& pin) {
        return static_cast<HardwareInterface*>(instance)->GPIO_Read(pin) ? 1 : 0;
    }

    void Hardware_GPIO_Write(HardwareInstance instance, const PinInfo& pin, int state) {
        static_cast<HardwareInterface*>(instance)->GPIO_Write(pin, static_cast<GPIO_State>(state));
    }
