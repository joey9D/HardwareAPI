									...
									CO_CANtx_t txArray[], uint16_t txSize, uint16_t CANbitRate)
{
    ...
    /***************************************/
    ((CANopenNodeSTM32*)CANptr)->HWInitFunction(((CANopenNodeSTM32*)CANptr)->baudrate);
    ...
    if (HAL_FDCAN_ActivateNotification(((CANopenNodeSTM32*)CANptr)->CANHandle,
                                       ...
                                       0xFFFFFFFF)
        != HAL_OK)// FDCAN_IT_RX_FIFO1_NEW_MESSAGE Deleted to allow for the use of seperate CAN RX FIFO for each message
...
static uint8_t prv_send_can_message(CO_CANmodule_t* CANmodule, CO_CANtx_t* buffer)
{
    ...
    if (HAL_CAN_GetTxMailboxesFreeLevel(((CANopenNodeSTM32*)CANmodule->CANptr)->CANHandle) > 0)
    {
        ...
        /* Now add message to FIFO. Should not fail */
        //success = HAL_CAN_AddTxMessage(((CANopenNodeSTM32*)CANmodule->CANptr)->CANHandle, &tx_hdr, buffer->data, &TxMailboxNum) == HAL_OK;
        HAL_StatusTypeDef err = HAL_CAN_AddTxMessage(((CANopenNodeSTM32*)CANmodule->CANptr)->CANHandle, &tx_hdr, buffer->data, &TxMailboxNum);
        if(err == HAL_OK)
        {
        	success = 1;
        }
        else
        {
			...
			case HAL_BUSY:
				break;
			case HAL_ERROR:
				switch(((CANopenNodeSTM32*)CANmodule->CANptr)->CANHandle->ErrorCode)
				{
