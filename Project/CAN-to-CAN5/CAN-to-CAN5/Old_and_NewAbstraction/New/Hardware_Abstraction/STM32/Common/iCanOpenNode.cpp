...
{
    canopenNodeSTM32->desiredNodeID = DEFAULT_CAN_NODE_ID;
	canopenNodeSTM32->activeNodeID = 0;
	canopenNodeSTM32->baudrate = 250;
	canopenNodeSTM32->timerHandle = nullptr; // Pass in the timer that is going to be used for generating 1ms interrupt for tmrThread function,
	canopenNodeSTM32->timerCall_in_us = DEFAULT_CAN_TIMERIRQ_IN_US;
	canopenNodeSTM32->CANHandle = nullptr; // Pass in the CAN Handle to this function and it will be used for all CAN Communications. It can be FDCan or CAN
	canopenNodeSTM32->HWInitFunction = nullptr; // Pass in the function that initialize the CAN peripheral, usually MX_CAN_Init( for CANFD it is MX_FDCAN_Init)
...
int CanOpenNodeSTM32::canopen_app_init()
{
...
*/
    ...
    HAL_TIM_Base_Start_IT(canopenNodeSTM32->timerHandle); //1ms interrupt // abstract this to the hardware layer
...
uint8_t CanOpenNodeSTM32::canopen_app_process()
{
    ...
    { // Make sure more than 1ms elapsed
        ...
        else if(reset_status == CO_RESET_APP)
        {
            ...
            HAL_NVIC_SystemReset(); // Reset the STM32 Microcontroller // todo abstract this to a function in Hardware layer
        }
    }
    return executed;
}
