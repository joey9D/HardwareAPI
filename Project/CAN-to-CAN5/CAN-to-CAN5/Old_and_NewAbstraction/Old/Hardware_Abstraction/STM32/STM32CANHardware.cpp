...
bool STM32CANHardware:: GPIO_Read(int pin) {
    return HAL_GPIO_ReadPin(GPIOA, pin) == GPIO_PIN_SET;
}

void STM32CANHardware:: GPIO_Write(int pin, bool value) {
    HAL_GPIO_WritePin(GPIOA, pin, value ? GPIO_PIN_SET : GPIO_PIN_RESET);
}

unsigned long STM32CANHardware:: GetTick() {
    return HAL_GetTick();
}
